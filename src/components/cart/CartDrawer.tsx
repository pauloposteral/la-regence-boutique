...
const CartDrawer = () => {
  ...
  const aplicarCupom = async () => {
    ...
    try {
      ...
      let desc = 0;
      if (data.desconto_percentual) {
        desc = subtotal * (Number(data.desconto_percentual) / 100);
      } else if (data.desconto_valor) {
        desc = Number(data.desconto_valor);
      }
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b border-border">
              ...
              <button onClick={closeCart} className="p-1 hover:bg-muted rounded transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center px-8">
                  ...
                  <p className="font-body text-sm text-muted-foreground mb-6">
                    Explore nossos cafés e encontre o sabor perfeito.
                  </p>
                  ...
                  {items.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <Link
                          to={`/cafe/${item.slug}`}
                          className="font-display text-sm font-semibold truncate block hover:text-accent transition-colors"
                          onClick={closeCart}
                        >
                          {item.nome}
                        </Link>
              ...
              <div className="border-t border-border p-5 space-y-4">
                ...
                {!cupom ? (
                  <div className="flex gap-2">
                    ...
                    />
                    <Button variant="outline" size="sm" onClick={aplicarCupom} disabled={cupomLoading} className="font-body text-xs shrink-0">
                      Aplicar
                    </Button>
                  </div>
                ) : (
                  <div className="flex items-center justify-between bg-accent/10 rounded px-3 py-2">
                    <span className="text-xs font-body font-medium text-accent">
                      Cupom: {cupom} (-R$ {desconto.toFixed(2).replace(".", ",")})
                    </span>
                    <button onClick={() => { setCupom(null); setDesconto(0); }} className="text-xs text-muted-foreground hover:text-destructive">
                ...
                <div className="space-y-1.5 text-sm font-body">
                  ...
                  <div className="flex justify-between text-muted-foreground">
                    <span>Frete</span>
                    <span>{subtotal >= FRETE_GRATIS_MIN ? "Grátis" : "Calculado no checkout"}</span>
                  </div>
                  <div className="flex justify-between font-semibold text-base pt-2 border-t border-border">
                    <span>Total</span>
                    <span>R$ {total.toFixed(2).replace(".", ",")}</span>
                  </div>
                  <p className="text-[10px] text-accent text-right">
                    R$ {(total * 0.9).toFixed(2).replace(".", ",")} no Pix (10% off)
                  </p>
                </div>
