...
const AdminCupons = () => {
  ...
  const openEdit = (c: any) => {
    setEditing(c);
    setForm({
      codigo: c.codigo, desconto_percentual: c.desconto_percentual, desconto_valor: c.desconto_valor,
      valor_minimo: c.valor_minimo, usos_restantes: c.usos_restantes,
      valido_ate: c.valido_ate ? c.valido_ate.slice(0, 10) : "", ativo: c.ativo,
    });
    setOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      await supabase.from("cupons").update(payload).eq("id", editing.id);
            ...
            {cupons.map((c: any) => (
              <tr key={c.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                ...
                <td className="px-4 py-3 font-body text-sm">
                  {c.desconto_percentual ? `${c.desconto_percentual}%` : c.desconto_valor ? `R$ ${Number(c.desconto_valor).toFixed(2)}` : "—"}
                </td>
